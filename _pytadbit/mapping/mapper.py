...
def iterative_mapping_gem(gem_index_path, fastq_path, out_sam_path,
    ...
    if bash_reader != 'cat':
        ...
        print 'Bash reader is not trivial, read input with %s, store in %s' % (' '.join(reading_command), converted_fastq)
        ...
        print 'Run iterative_mapping recursively on %s' % converted_fastq
    ...
    if min_seq_len <= local_seq_end - seq_start:
        ...
        cutoff = 0.04
        try:
            print 'Reading command: %s' % ' '.join(reading_command)
            # pipeline.append(
        ...
        # Recursively go to the next iteration.
        print 'Save unique aligments and send the non-unique ones to the next iteration'
