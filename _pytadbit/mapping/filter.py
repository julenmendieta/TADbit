...
def filter_reads(fnam, frag_count, max_molecule_length=500,
    ...
    :param frag_count: dictionary with the coordinates of the restriction enzyme
       (RE) fragments. Generated by
       :func:`pytadbit.mapper.restriction_enzymes.map_re_sites`
    ...
    """
    masked = {'1- self-circle'        : set(), 
              '2- dangling-end'       : set(),
              '3- extra dangling-end' : set(),
              '4- error'              : set(),
              '5- duplicated'         : set(),
              '6- too close from RE'  : set(),
              '7- too short'          : set(),
              '8- too large'          : set(),
              '9- over-represented'   : set()}
    uniq_check = {}
    ...
    for line in open(fnam):
        read, cr1, ps1, sd1, rs1, re1, cr2, ps2, sd2, rs2, re2 = line.split()
        uniq_key = tuple(sorted((cr1 + ps1, cr2 + ps2)))
        if not uniq_key in uniq_check:
            uniq_check[uniq_key] = read
        else:
            masked['5- duplicated'].add(read)
            if not uniq_check[uniq_key] in masked['5- duplicated']:
                masked['5- duplicated'].add(uniq_check[uniq_key])
        ...
        if re1 == re2:
            if sd1 != sd2:
                if (ps2 > ps1) == sd2:
                    # ----<===---===>---
                    masked['1- self-circle'].add(read)
                else:
                    # ----===>---<===---
                    masked['2- dangling-end'].add(read)
            else:
                # ---===>--===>--- or ---<===--<===---
                masked['4- error'].add(read)
        elif (abs(ps1 - ps2) < max_molecule_length
              and ps1 != ps2
              and ps2 > ps1 != sd2):
            # different fragments but facing and very close
            masked['3- extra dangling-end'].add(read)
        elif ((abs(re1 - ps1) < re_proximity) or
              (abs(rs1 - ps1) < re_proximity) or 
              (abs(re2 - ps2) < re_proximity) or
              (abs(rs2 - ps2) < re_proximity)):
            masked['6- too close from RE'].add(read)
        elif ((re1 - rs1) < min_frag_size) or ((re2 - rs2) < min_frag_size) :
            masked['7- too short'].add(read)
        elif ((re1 - rs1) > max_frag_size) or ((re2 - rs2) > max_frag_size):
            masked['8- too large'].add(read)
        elif frag_count[(cr1, rs1)] > cut or frag_count[(cr2, rs2)] > cut:
            masked['9- over-represented'].add(read)
    del(uniq_check)
    return masked
